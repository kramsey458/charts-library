...
The script applies stealth hardening (`playwright-stealth` when available plus manual browser fingerprint tweaks), opens TradingView's symbol search explicitly from the header symbol control, and uses deterministic timing tuned for faster throughput while still waiting for each chart to finish loading before capture.
...
- `POST_NAVIGATION_WAIT_MS=900` wait after navigation events (initial login/chart transitions).
- `SEARCH_RESULTS_WAIT_MS=180` wait after typing ticker before pressing Enter.
- `SYMBOL_LOAD_TIMEOUT_MS=12000` max wait for chart title to reflect the selected ticker.
- `CHART_RENDER_WAIT_MS=900` extra settle delay after ticker switch to ensure image-ready chart render.
- `INTER_TICKER_DELAY_MS=80` gap between completed download and next ticker.
